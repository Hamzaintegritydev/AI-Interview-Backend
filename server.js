...
async function getAIResponse(prompt) {
  try {
    ...
    return "Could you tell me about your experience?";
  }
}
...
app.post('/start-interview', async (req, res) => {
  ...
  const prompt = `You're conducting a live interview for a ${jobPosition} (${experienceLevel} level). Start with a basic background question for the candidate such as 'Tell me about yourself', 'Why are you interested in this role?', or 'Walk me through your resume'. Return only the question.`;
...
app.post('/process-answer', async (req, res) => {
  const { answer } = req.body;
  if (answer) currentInterviewData.answers.push(answer);

  const now = new Date();
  const interviewDuration = (now - currentInterviewData.interviewStartTime) / (1000 * 60);
  if (interviewDuration >= 10) {
    currentInterviewData.interviewEndTime = now;
    const endMsg = "Thank you for your time. The interview is now complete.";
    const audio = await generateSpeech(endMsg);
    return res.json({ isComplete: true, message: endMsg, audio });
  }
  ...
  const prompt = `You're interviewing for a ${currentInterviewData.jobPosition} (${currentInterviewData.experienceLevel}).
Here is the transcript so far:
${context}
Ask the next logical question. Make it specific, natural, and engaging. No extra explanation—just return the question.`;
  ...
  res.json({
    question,
    audio: audioBase64,
    remainingTime: Math.max(0, 10 - interviewDuration).toFixed(1),
    questionCount: currentInterviewData.questions.length
  });
});
...
app.post('/complete-interview', async (req, res) => {
  ...
  let transcript = `Interview for ${currentInterviewData.jobPosition} (${currentInterviewData.experienceLevel})\n`;
  transcript += `Date: ${currentInterviewData.interviewStartTime.toISOString()}\n`;
  transcript += `Duration: ${((currentInterviewData.interviewEndTime - currentInterviewData.interviewStartTime) / 60000).toFixed(2)} minutes\n\n`;
  ...
  const evaluationPrompt = `
You are an expert interviewer. Evaluate the following transcript of a ${currentInterviewData.jobPosition} interview (${currentInterviewData.experienceLevel} level):
...
Provide a JSON response with:
{
  ...
  "strengths": [{"strength": "string", "example": "string"}],
  "improvementAreas": [{"area": "string", "example": "string"}],
  "technicalAssessment": {"score": number, "analysis": "string"},
  "communicationAssessment": {"score": number, "analysis": "string"},
  "culturalFitAssessment": {"score": number, "analysis": "string"},
  "problemSolvingAssessment": {"score": number, "analysis": "string"},
  "standoutMoments": [{"type": "positive|negative", "moment": "string", "impact": "string"}],
  "recommendation": "Reject|Consider|Strong Consider|Hire",
  "feedback": "paragraph",
  "developmentPlan": ["item1", "item2", "item3"]
}`;

  let evaluationRaw = await getAIResponse(evaluationPrompt);

  // Remove triple backticks if present
  evaluationRaw = evaluationRaw.replace(/```json\s*|\s*```/g, '');
  let evaluation;
  try {
    evaluation = JSON.parse(evaluationRaw);
  } catch (e) {
    console.error('JSON parse error:', e.message);
    evaluation = { error: "Failed to parse AI evaluation." };
  }

  const thankYouMsg = "Thank you for completing the interview. A full evaluation has been generated.";
  const audio = await generateSpeech(thankYouMsg);
